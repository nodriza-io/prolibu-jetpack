...
async function importData(domain, collection, format, query = 'select=-workspace -createdBy -updatedBy -createdAt -updatedAt') {
  let open;
      ...
      allData.forEach(async (record) => {
        if (record.keyname) {
          ...
          if (record.jsonData) {
            ...
            if (!fs.existsSync(jsonDataSavePath)) {
          ...
          if (!_.isEmpty(record.figmaFileId)) {
