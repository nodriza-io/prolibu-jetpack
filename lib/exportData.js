...
async function autoExportData(filepath) {
  // Extract domain and collection from the provided filepath
  const pathSegments = filepath.split(path.sep);
  const domain = pathSegments[pathSegments.indexOf('accounts') + 1];
  const collection = pathSegments[pathSegments.indexOf('data') + 1];
  const format = path.extname(filepath).slice(1); // Extract the file extension without the dot
  ...
  // If there are changes, save them to a "changes" file in the appropriate directory
  const changesPath = path.join(path.dirname(filepath), `changes.${format}`);
  // Logic to determine changes and write them goes here
  // ...
